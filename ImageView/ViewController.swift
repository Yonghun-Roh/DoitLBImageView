...
class ViewController: UIViewController {
    ...
    @IBAction func btnResizeImage(_ sender: UIButton) {
        ...
        if (isZoom) {   //true 현재 확대된 그림일 경우(즉, 타이틀은 축소)
            ...
            newHeight = imgView.frame.height / scale      // 이미지 뷰의 프레임 높이값을 scale로 나눔
            ...
            newHeight = imgView.frame.height * scale    //이미지 뷰의 프레임 높이값을 scale로 곱함
            
            btnResize.setTitle("축소", for: .normal)      // 버튼의 타이틀을 "축소"로 변경함
        }
            imgView.frame.size = CGSize(width: newWidth, height: newHeight)     // 이미지 뷰의 프레임 크기를 수정된 너비와 높이로 변경합니다
